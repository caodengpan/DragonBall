    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i):SV_Target
            {
				...
				fixed3 ramp = tex2D(_Ramp,float2(diff,diff));
				ramp = lerp(_SColor.rgb,_Color.rgb,ramp);
                fixed3 diffuse=_LightColor0.rgb*albedo *ramp.rgb;
			    ...
			    half rim = 1.0f - saturate( dot(viewDir, worldNormal) );
			    rim = smoothstep(_RimMin, _RimMax, rim);
                return fixed4(ambient+diffuse+specular + (_RimColor.rgb * rim) * _RimColor.a,1.0);
            }
            ENDCG    
        }
    }
